(ns saml-backend.handler-test
  (:require
    [clojure.test :refer :all]
    [ring.mock.request :refer :all]
    [saml-backend.handler :refer :all]
    [saml-backend.middleware.formats :as formats]
    [muuntaja.core :as m]
    [mount.core :as mount]))
...
(use-fixtures
  :once
  (fn [f]
    (mount/start #'saml-backend.config/env
                 #'saml-backend.handler/app-routes)
    (f)))
